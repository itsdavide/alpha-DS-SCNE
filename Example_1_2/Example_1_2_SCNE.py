...
for alpha in alphas:
    ...
    # Gradient vector
    grad = m_nu_orig**2 + (np.array([2, 4, 6]).reshape((len(m_nu_orig), 1))) * m_nu_orig 
    ...
    print('m_mu_rec:', np.round(m_mu, 4), 'with sum = ', round(np.sum(m_gamma), 4))
    print('m_nu_rec:', np.round(m_nu, 4), 'with sum = ', round(np.sum(m_gamma), 4))
    print()
