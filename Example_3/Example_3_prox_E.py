...
def prox_E(q, I, epsilon): 
    ...
    model.c2 = pyo.Constraint(expr=(1/3) * sum(model.p[i]**3 for i in model.I) + sum((i+1) * model.p[i]**2 for i in model.I) + model.p[0] * model.p[1] + 2 * model.p[0] * model.p[2] + model.p[1] * model.p[2] == model.E)
